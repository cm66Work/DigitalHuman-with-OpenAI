...
namespace TextSpeech
{
    public class TextToSpeech : MonoBehaviour
    {
        ...
        public Action<byte[]> onAudioAvailableCallBack;
        public Action<int, int, int> onBeginSynthesisCallBack;
        ...
        public void onDoneFile(string message)
        {
            if(onDoneCallback != null)
                onDoneCallback();
        }
        public void onBeginSynthesis(string data)
        {
            // returns data needed for unity to create audio clips returned by onAudioAvailable
            if(onBeginSynthesisCallBack == null) return;

            string[] splitData = data.Split(",");

            onBeginSynthesisCallBack(int.Parse(splitData[1]), int.Parse(splitData[3]), int.Parse(splitData[2]));
        }
        public void onAudioAvailable(string data)
        {
            if(onAudioAvailableCallBack == null) return;

            string[] encodedArray = data.Split(',');
            onAudioAvailableCallBack(Convert.FromBase64String(encodedArray[1]));
        }
